import { View, Text, Image, StyleSheet, ImageStyle, ViewStyle, Dimensions, TextStyle } from "react-native";
...
const ProfileScreen = ({ navigation }: ProfileScreenProps): JSX.Element => {
    ...
    if (isLoading) return <View><Text>Loading user...</Text></View>;

    if (isError || !user) return <View><Text>Error loading user!</Text></View>;

    const defeats = user.game_stats.matches - user.game_stats.victories;
    const victoriesPercentage = user.game_stats.victories > 0 ? ((user.game_stats.victories / user.game_stats.matches) * 100).toPrecision(2) : 0
    const defeatsPercentage = defeats > 0 ? ((defeats / user.game_stats.matches) * 100).toPrecision(2) : 0
    ...
    return (
        <BaseLayout>
            ...
            <View style={styles.outerLevelBar}>
                <View style={{...styles.innerLevelBar, width: user.game_stats.xp_points}}>
                </View>
            </View>
            ...
            <View style={styles.statContainer}>
                <Text style={styles.statTitle}>Victories <Text style={styles.statTitleInfo}>({victoriesPercentage}%)</Text></Text>
                <Text style={{...styles.statValue, color: "#5AE381"}}>{user.game_stats.victories}</Text>
            </View>

            <View style={{...styles.statContainer, borderBottomWidth: 0}}>
                <Text style={styles.statTitle}>Defeats <Text style={styles.statTitleInfo}>({defeatsPercentage}%)</Text></Text>
