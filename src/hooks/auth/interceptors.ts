...
const refreshTokenThenRetryRequest = async (
  responseError: AxiosError<ErrorResponse>,
  clearUser: () => void,
) => {
  const isNotUnauthorized: boolean = responseError.response?.status !== 401;
  const isNotAccessTokenExpired: boolean =
    responseError.response?.data.cause.data !== JwtCause.ACCESS_TOKEN_EXPIRED;
...
const useAuthInterceptors = (clearUser: () => void) => {
  useEffect(() => {
    const requestInterceptor =
      API.interceptors.request.use(attachTokenInRequest);

    const responseInterceptor = API.interceptors.response.use(
      (response: AxiosResponse) => response,
      (responseError: AxiosError<ErrorResponse>) =>
        refreshTokenThenRetryRequest(responseError, clearUser),
    );
